    ...
    pub(crate) unsafe fn from_native(
        ...
        unsafe {
            ...
            match event_type {
                ...
                // Trackpad pinch arrives as NSEventTypeMagnify — a gesture type,
                // not a scroll event, which stock GPUI 0.2 drops. Synthesize a
                // cmd-flagged vertical pixel scroll so hosts can route pinch
                // through their existing "cmd+scroll = zoom at cursor" path.
                // Scale 500 makes a host zoom factor exp(-dy*k) land on
                // exp(magnification) ≈ 1 + magnification, i.e. fingers-true.
                NSEventType::NSEventTypeMagnify => window_height.map(|window_height| {
                    let mag = native_event.magnification() as f32;
                    Self::ScrollWheel(ScrollWheelEvent {
                        position: point(
                            px(native_event.locationInWindow().x as f32),
                            window_height - px(native_event.locationInWindow().y as f32),
                        ),
                        delta: ScrollDelta::Pixels(point(px(0.0), px(-mag * 500.0))),
