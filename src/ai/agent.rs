...
- add_canvas(x?, y?, w?, h?, background?, strokes?)：放一块位图画布并画上像素级笔触（水彩晕染/干笔飞白——矢量工具画不出的效果）。strokes 每笔给 points（世界坐标，≥2 点）+ brush（watercolor 水彩分层晕染 / ink 实心墨迹 / dry_brush 飞白颗粒）+ color/width。笔触自动裁剪在画布内，多层透明叠色成立。适合水彩天空、水墨晕染、氛围小品；不需要像素效果时用普通矢量工具。
